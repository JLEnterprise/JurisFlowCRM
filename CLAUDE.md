...
(Obs.: `docs/agente-advjuris/` NÃO é guia de dev — é o material jurídico do agente AdvJuris usado dentro do app.)

## Visão geral
CRM jurídico SaaS multi-escritório (multi-tenant) para escritórios de advocacia.
Interface e textos em **português do Brasil**.

- **Pasta local:** `D:\Projetos\Claude\JurisFlow-ADV`
...
src/
  ...
  data/, utils/
public/                    # estáticos (logo.png)
supabase/
  migrations/              # migrações SQL (AAAAMMDD_descricao.sql) — mudanças novas entram aqui
  sql-legado/              # SQL antigo do Antigravity, só referência (ver README da pasta)
docs/
  ARQUITETURA_MULTI_TENANT.md
  agente-advjuris/         # prompt, regras e skills jurídicas do AdvJuris (conteúdo, não dev)
.claude/skills/            # skills do Claude Code (supabase, supabase-postgres-best-practices)
INICIAR_JURISFLOW.bat      # atalho: npm run dev + abre o navegador
PUBLICAR.bat               # atalho: build + commit + push (deploy Vercel)
```
...
- O Claude edita os arquivos direto em `D:\Projetos\Claude\JurisFlow-ADV`.
...
Tudo o que existia antes da migração (zip de backup, pasta `Backup_JurisFlow/`, scripts `check_*.js`,
`purge_supabase.js`, `write_crm.cjs`, `netlify.toml`, `agente.md` etc.) está no GitHub na branch
`backup/antigravity-2026-09-24` e na tag `antigravity-final`. Para recuperar um arquivo:
