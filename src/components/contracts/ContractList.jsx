...
export function ContractList({ onOpenNewContract, onEditContract, onSelectContract, onOpenContractDetail, onNavigate, onSignContract }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Busca, filtros e ações numa linha só */}
      <div className="flex flex-wrap items-center gap-2.5">
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            placeholder="Buscar número, cliente, objeto..."
        ...
        <div className="ml-auto flex items-center gap-2">
          {/* Anexar contrato pronto (PDF/Word) */}
          <button
            onClick={() => setQuickAttachOpen(true)}
            className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 dark:border-white/[0.08] px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:border-gold-500/40 transition-colors"
            title="Anexar contrato pronto, lauda ou minuta do escritório em PDF ou Word"
          >
            <Paperclip className="h-3.5 w-3.5 text-gold-600 dark:text-gold-400" /> Anexar
          </button>
